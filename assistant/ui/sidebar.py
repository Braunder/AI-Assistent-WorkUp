...
def _render_tts_controls() -> None:
    st.subheader("🔊 Озвучка")
    st.checkbox(
        "Включить озвучку ответов",
        key="web_tts_enabled",
        help="Озвучиваются только текстовые части ответа. Код и символный шум фильтруются.",
    )
    st.checkbox(
        "Автопроигрывание нового ответа",
        key="web_tts_autoplay",
        help="Браузер может блокировать autoplay; тогда нажмите кнопку 'Озвучить' у сообщения.",
    )
