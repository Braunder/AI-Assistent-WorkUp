...
from assistant.tools.roadmap_validator import (
    get_random_roadmap_question,
    get_roadmap_coverage,
...
class MLAssistant:
    ...
    def _handle_local_command(self, user_input: str) -> str | None:
        ...
        if lower in ("/help", "/?"):
            return (
                "Команды:\n"
                "- /mode study|diagnostic|interview|status\n"
                "- /roadmap status — показать прогресс по roadmap категориям\n"
                "- /roadmap <category> — вопрос из категории (math, nlp, supervised_learning, model_evaluation, deep_learning, production)\n"
                "- /roadmap all — случайный вопрос из всех категорий"
            )
        ...
        if lower.startswith("/roadmap"):
            ...
            if len(parts) == 1:
                return "Используй: /roadmap status | /roadmap <category> | /roadmap all"

            subcommand = parts[1].strip()
            if subcommand == "status":
                coverage = get_roadmap_coverage()
                lines = ["📊 Roadmap.sh ML Engineer Coverage:"]
                total = 0
                for cat in sorted(coverage.keys()):
                    count = coverage[cat]
                    total += count
                    lines.append(f"  {cat}: {count} вопросов")
                lines.append(f"  **Всего: {total} вопросов**")
                return "\n".join(lines)

            # Get a question from category
            try:
                if subcommand == "all":
                    category = None
                else:
                    category = subcommand
                question, context = get_random_roadmap_question(category=category)
                return f"{question}\n\n**Контекст:** {context}"
            except ValueError as e:
                coverage = get_roadmap_coverage()
                available = ", ".join(sorted(coverage.keys()))
    ...
    async def verify_claim(self, claim: str) -> str:
        ...
        web_result = search_web(claim, max_results=3)
