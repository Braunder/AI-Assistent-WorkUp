...
## Linear Algebra for RAG and Embeddings
Source: https://github.com/3b1b/essence-of-linear-algebra
Source: https://www.khanacademy.org/math/linear-algebra
Source: https://distill.pub/2016/misunderstanding-umap/

Embeddings are dense vectors; RAG retrieval relies on vector similarity.
Key concepts: dot product (cosine similarity), norms (l2 normalization), rank, singular value decomposition (SVD).
In production, understand why cosine similarity works for high-dimensional spaces and why l2-normalized embeddings enable efficient scaling.
Common interview trap: confuse Euclidean distance with cosine similarity — they behave differently in high dimensions (cosine is preferred for embeddings).
Practical skill: diagnose embedding quality via singular values, understand PCA/UMAP for visualization, explain why dimension mismatch breaks retrieval.

## Probability and Statistics for ML
Source: https://www.deeplearningbook.org/contents/prob.html
Source: https://github.com/rbharath/courses/tree/master/statistics

Bayes' theorem: P(H|E) = P(E|H)P(H) / P(E) — foundation for probabilistic reasoning in LLM inputs.
Distributions matter: normal distribution (Gaussian) for model outputs, zipfian for token frequencies, exponential for inter-arrival times in batch processing.
Key metrics: mean, variance, standard deviation — understanding model output distributions helps debug unexpected behaviors.
Cross-validation: k-fold validates that models generalize; leave-one-out (LOOCV) exhaustive but expensive.
Interview expectation: explain why data is distributed, not deterministic, and why statistical tests require assumptions.

## NLP Fundamentals: Tokenization, Embeddings, Attention
Source: https://github.com/huggingface/huggingface-nlp
Source: https://www.3blue1brown.com/topics/transformers
Source: https://arxiv.org/abs/1706.03762 ("Attention is All You Need")

Tokenization splits text into subword units (BPE, WordPiece, SentencePiece); affects context window, embedding quality, and cost.
Embeddings map tokens to dense vectors; typically learned during pretraining (e.g., Word2Vec, GloVe, contextual BERT embeddings).
Attention mechanism computes relevance weights between query and key-value pairs; enables models to focus on relevant context.
Self-attention: compare token to all tokens in sequence; multi-head attention: parallel attention with different subspaces for diversity.
RAG relevance: transformer embeddings (BERT, ada, sentence-transformers) are state-of-art for retrieval because they capture semantic similarity.
Interview skill: explain why positional encoding matters (models need to distinguish token position), why attention is differentiable (backprop works), why GPT models are autoregressive (predict next token given past tokens).
...
Source: https://github.com/chiphuyen/machine-learning-systems-design

Regression predicts continuous values (e.g., click probability, latency); loss function typically MSE.
Classification predicts discrete classes; loss function typically cross-entropy (for probabilities) or hinge loss (for margins).
Linear regression: y = Xw + b; closed-form solution exists if X is invertible (rarely true at scale — use SGD instead).
Logistic regression: sigmoid activation on linear model; calibrated probabilities unlike SVM.
Regularization (L1/L2) prevents overfitting by penalizing large weights; critical for high-dimensional problems.
Bias-variance tradeoff: underfitting (high bias), overfitting (high variance); validation curve shows trade-off graphically.
Interview expectation: know when to use each model type, explain regularization effect, discuss computational complexity.
...
Source: https://www.comet.com/guides/model-evaluation-metrics/

Precision = TP/(TP+FP): false alarm rate matters when cost of incorrect positive is high (e.g., fraud detection).
Recall = TP/(TP+FN): miss rate matters when cost of false negative is high (e.g., cancer detection).
F1 = 2*precision*recall/(precision+recall): harmonic mean for imbalanced datasets.
ROC-AUC: area under receiver operating characteristic curve; useful for ranking (e.g., recommenders) where threshold is flexible.
Confusion matrix: visualizes TP, FP, TN, FN; helps diagnose which classes are confused.
For RAG systems specifically: track retrieval metrics (NDCG, MAP) separately from generation metrics (BLEU, ROUGE, semantic similarity).
Interview expectation: choose metrics aligned with business goal; explain why accuracy is misleading for imbalanced data; validate on held-out test set without lookahead bias.

## Deep Learning Foundations
Source: https://www.deeplearningbook.org/
Source: https://course.fast.ai/
Source: https://github.com/aman-garg/deep-learning-notes

Backpropagation: efficient algorithm for computing gradients via chain rule; enables learning of multi-layer networks.
Gradient descent: iterative optimization; variants include SGD (noisy but escapes local minima), momentum (accelerates convergence), Adam (adaptive learning rates per parameter).
Activation functions: ReLU (rectified linear) most common in modern networks; sigmoid/tanh in older architectures; enables non-linear function approximation.
Batching: mini-batches reduce variance of gradient estimates; larger batches → faster training but may lose generalization (sharp minima).
Regularization in deep learning: dropout (random neuron masking during training), batch normalization (normalize layer inputs), weight decay (L2 penalty).
Loss functions: MSE for regression, cross-entropy for classification, contrastive loss for embeddings.
Interview expectation: explain why deep networks can approximate arbitrary functions, discuss optimization challenges (vanishing gradients, local minima), know trade-offs of common design choices.

## Conclusion: Roadmap.sh ML Engineer Alignment
This knowledge base bridges ML theory and LLM production engineering.
Study phase should cover math and supervised/unsupervised learning; diagnostic mode validates depth.
Production phase requires RAG, serving, and observability knowledge specific to LLM systems.
A strong middle LLM engineer masters both: statistical rigor of evaluation + operational reliability of serving.
...
- https://github.com/3b1b/essence-of-linear-algebra
- https://www.khanacademy.org/math/linear-algebra
- https://distill.pub/2016/misunderstanding-umap/
- https://www.deeplearningbook.org/contents/prob.html
- https://github.com/rbharath/courses/tree/master/statistics
- https://github.com/huggingface/huggingface-nlp
- https://www.3blue1brown.com/topics/transformers
...
- https://www.comet.com/guides/model-evaluation-metrics/
- https://www.deeplearningbook.org/
- https://course.fast.ai/
- https://github.com/aman-garg/deep-learning-notes
