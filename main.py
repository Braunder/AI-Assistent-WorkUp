...
from assistant.core.tts_engine import TTSEngine
...
def _build_tts_engine() -> TTSEngine:
    return TTSEngine(
        ...
        use_streaming=settings.tts_use_streaming,
        stream_sample_rate=settings.tts_stream_sample_rate,
        cache_enabled=settings.tts_cache_enabled,
        cache_max_items=settings.tts_cache_max_items,
        cache_max_text_len=settings.tts_cache_max_text_len,
        cache_chunk_bytes=settings.tts_cache_chunk_bytes,
        stream_emit_chunk_bytes=settings.tts_stream_emit_chunk_bytes,
        max_input_chars=settings.tts_max_input_chars,
        min_chunk_chars=settings.tts_min_chunk_chars,
    )


async def speak_answer(tts_engine: TTSEngine, text: str) -> None:
    """Play assistant response through TTS, skipping non-speech technical fragments."""
    spoken_preview = tts_engine.prepare_text_for_tts(text)
    if not spoken_preview:
        return

    stream: sd.RawOutputStream | None = None
    current_rate: int | None = None
    try:
        async for sample_rate, chunk in tts_engine.stream_pcm(text):
            if not chunk:
                continue
...
async def run_cli() -> None:
    ...
    tts_engine = _build_tts_engine() if settings.tts_enabled else None
    cfg = VoiceConfig()
    ...
    print(f"TTS: {'ON' if tts_engine is not None else 'OFF'}")
    print(
        "Автосохранение: "
    ...
    try:
        while True:
            ...
            user_text = user_text
            answer_text = await stream_answer(assistant, user_text)
            if tts_engine is not None:
                await speak_answer(tts_engine, answer_text)
